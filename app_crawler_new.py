...
def getAppInfo(app_name,app_id,url1,url2,file):
...
if __name__ == "__main__":
    ...
    for appstore_name in search_entries:
        ...
        for keyword in keywords:
            ...
            if(len(app_id)>0):
                getAppInfo(keyword,app_id,"http://shouji.baidu.com/software/",".html",resfile)
            else:
                #notfoundfile
                f = open(notfoundfile, 'a')
                f.write(str(keyword))
                f.write("\n")
                f.close()                    
